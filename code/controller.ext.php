...
class module_controller {
    ...
    static function DisplayDefaultRecords() {
        ...
        $line .= "<form action=\"./?module=cdns_manager&action=CreateDefaultRecords\" method=\"post\">";
    ...
    static function DisplayRecords() {
        ...
        $line .= "</div>";
        $line .= "</div>";
        $line .= "<form action=\"./?module=cdns_manager&action=SaveDNS\" method=\"post\">";
...
//          $sql = $zdbh->prepare("SELECT * FROM x_dns WHERE dn_acc_fk=" . $currentuser['userid'] . " AND dn_type_vc='NS' AND dn_vhost_fk=" . $domainID . " AND dn_deleted_ts IS NULL ORDER BY dn_host_vc ASC");
        ...
        $line .= self::getCSFR_Tag();
        $line .= "</form>";
        $line .= "</div>"; // will this closing div needed to be uncommented.. (or how its called if you remove the // at the beginning..)
        $line .= "<!-- END DNS FORM -->";
        $line .= "<div class=\"zgrid_wrapper\">";
    ...
    static function DisplayDomains() {
        ...
        $line .= "<form name=DisplayDNS action=\"./?module=cdns_manager&action=DisplayRecords\" method=\"post\">";
        $line .= "<br><br>";
        $line .= "<table class=\"zform\">";
        $line .= "<tr>";
    ...
    static function CheckZoneRecord($domainID) {
        ...
        if ($numrows->execute()) {
            if ($numrows->fetchColumn() <> 0) {
				...
				$checkline .= "@ IN SOA ns1." . $domain['dn_name_vc'] . ".    ";
				$checkline .= "postmaster." . $domain['dn_name_vc'] . ". (" . fs_filehandler::NewLine();
